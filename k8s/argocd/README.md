...
helm upgrade --install argocd argo/argo-cd --version <pin> -n argocd \
  --create-namespace -f k8s/argocd/argocd-values.yaml \
...
`k8s/argocd/apps/*.yaml` — one Argo Application per platform component.
`k8s/argocd/root-app.yaml` (app-of-apps) will point at that directory so
the whole platform is one declarative tree (P8-5).
