...
| 13 | Mesh & routing | Istio (sidecar) implementing Gateway API; dev/prod host routing via nip.io |
...
→ Argo syncs **dev** → verify `dev.<LB-IP>.nip.io` → **promotion PR** to `envs/prod/`
→ review → merge → Argo syncs **prod** at `prod.<LB-IP>.nip.io`.
...
| 6 | Gateway API CRDs (K8s doesn't ship them) + Istio ≥1.26 manual Helm (**gateway = pinned NodePort 30080/30443 — default LoadBalancer type would bill a 2nd OCI LB**; istiod 200m/512Mi, global proxy 25m/64Mi); Gateway resources in manual mode; TF: TCP-443 passthrough listener (no cert needed — Istio terminates TLS); lab-CA wildcard cert; nip.io hosts | curl both hosts (--cacert); NO new LB in OCI console |
| 7 | kube-prometheus-stack sized (5d/6GB retention, local-path 8Gi, KCM/scheduler `--bind-address=0.0.0.0` fix on cp + in git config) + 4 NSG scrape rules + Istio ServiceMonitor/PodMonitor + Kiali + metrics-server; Alertmanager → Slack/587 (port 25 blocked). **7b**: logging = Loki SingleBinary (local-path, 72h) + Fluent Bit (CRI parser, cp toleration) + Grafana datasource | test alert received; logs queryable in Grafana |
