...
| 8 | ✅ **DONE 2026-07-14** (`e5ba00c`→`ba4082e`). As planned: trimmed Argo, annotation tracking, exact-version pins → annotation-only first syncs, ServerSideApply for kps AND istio-base (big CRDs), sync-waves (base 0 → istiod 1 → gateways 2), helm release secrets deleted after each adoption. **As-built additions**: read-only SSH deploy key for the private repo; helm.releaseName REQUIRED on every adoption (Argo defaults it to the app name → duplicate subchart resources; caught pre-sync); controller needs 1Gi limit (512Mi OOM-wedged syncs the moment kps arrived); ignoreDifferences for istio webhook failurePolicy (runtime Ignore→Fail flip) + kps caBundle; default v1/Endpoints exclusion overridden (kps static-pod scrape Endpoints must be managed for clean rebuilds); **Kiali NOT adopted** (random signing_key = non-deterministic render; stays Helm-managed); platform-root app-of-apps + fleet-wide automated+selfHeal (prune off on CRD carriers + root). ccm-csi stays Ansible-owned (GitOps horizon; hub-spoke = the fleet-scale alternative) | 10/10 apps Synced/Healthy; drift demo validated |
